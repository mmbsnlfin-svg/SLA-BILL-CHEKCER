...
# 9) NEW: MTTR 25% cap calculated after deducting gross Availability Penalty from bill value
# ============================================================
...
def _pdf_escape_line(line):
    """Escape text for ReportLab Paragraph while preserving alignment spaces."""
    line = str(line)
...
def _highlight_pdf_line(line):
    """Apply color/bold formatting to important note lines."""
    red_keywords = [
    ...
    escaped = _pdf_escape_line(line)
    raw = str(line)

    if any(k in raw for k in red_keywords):
        return f'<font color="#c00000"><b>{escaped}</b></font>'

    if any(k in raw for k in blue_keywords):
        return f'<font color="#0050a4"><b>{escaped}</b></font>'
...
def create_pdf_note(output_path, title, text_body):
    """
    Create clean portrait A4 PDF note from existing note text.
    Important lines are highlighted in red/blue as requested.
    """
    ...
    story.append(HRFlowable(width="100%", thickness=1.0, color=colors.HexColor("#0b2d6b")))
    story.append(Spacer(1, 9))

    # Build line-by-line paragraphs so selected lines can be bold/colored.
    for raw_line in str(text_body).split("\n"):
        if raw_line.strip() == "":
            story.append(Spacer(1, 5))
        else:
            story.append(Paragraph(_highlight_pdf_line(raw_line), mono_style))

    story.append(Spacer(1, 12))
    story.append(HRFlowable(width="100%", thickness=0.5, color=colors.lightgrey))
    story.append(Spacer(1, 6))
    story.append(Paragraph("Generated by BSNL SLA Bill Checker", footer_style))
...
def process_sla(
    ...
    # ---------- MTTR 25% cap ----------
    # Tender logic as clarified:
    # 1) Availability Penalty is deducted first from monthly service value.
    # 2) Cap amount is calculated WITHOUT considering any exemption.
    #    Therefore gross Availability Penalty is used for cap base.
    # 3) MTTR cap is applied on gross MTTR penalty, then MTTR exemption is deducted.
    # Example: Basic Rs.100 - Availability Gross Penalty Rs.10 = Rs.90; cap = 25% of Rs.90.
    total_basic_sla = round(float(routes["SLA_Charges_Rs"].sum()), 2)
    mttr_cap_base_after_availability = round(total_basic_sla - availability_penalty_gross, 2)
    ...
    mttr_gross_after_cap = round(min(mttr_gross, mttr_cap_25pct), 2)
    mttr_cap_applied = "YES" if mttr_gross > mttr_cap_25pct else "NO"

    # Exemption is deducted after cap so cap amount remains without exemption.
    mttr_net_after_cap = round(max(mttr_gross_after_cap - mttr_exempt, 0.0), 2)
...
5) SLA Penalty Deduction (as per tender):
   ...
   MTTR Cap Base after Gross Availability Penalty                  = Rs. {fmt_money(mttr_cap_base_after_availability)}
   MTTR 25% Cap Amount (before exemption)                          = Rs. {fmt_money(mttr_cap_25pct)}
   MTTR Penalty (Cap on gross, then exemption)                    = Rs. {fmt_money(mttr_net_after_cap)}   (Cap Applied: {mttr_cap_applied})
   -------------------------------------------------------------------------------
...
2. MTTR FAULTS Penalty Details                                   : (Code generated from Format-C)
   ...
   MTTR Cap Base after Gross Availability Penalty Rs.              : Rs. {fmt_money(mttr_cap_base_after_availability)}
   Max 25% MTTR Penalty Rs. ((RKM*Rate - Gross Availability Penalty)*25%) : Rs. {fmt_money(mttr_cap_25pct)}

   Slab wise faults and penalty:
{chr(10).join(mttr_lines)}

   Total MTTR Net penalty (Cap on gross, then exemption)           : Rs. {fmt_money(mttr_net_after_cap)}
...
"""
    ...
    summary = pd.DataFrame([
        ...
        ["MTTR cap 25% without exemption", mttr_cap_25pct],
        ["MTTR gross after cap", mttr_gross_after_cap],
