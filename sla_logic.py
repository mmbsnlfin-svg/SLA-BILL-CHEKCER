...
def create_pdf_note(output_path, title, text_body):
    ...
    This preserves the existing office note wording and calculation logic,
    while avoiding Notepad/font/alignment issues on different PCs.
    """
    ...
    story.append(HRFlowable(width="100%", thickness=0.8, color=colors.HexColor("#0b2d6b")))
    story.append(Spacer(1, 10))

    # maxLineLength wraps long lines so PDF remains readable on A4.
    story.append(Preformatted(str(text_body), mono_style, maxLineLength=112))
...
def process_sla(
    ...
    # Availability Penalty is deducted first from the monthly service value,
    # and 25% MTTR cap is calculated on the balance amount.
    # Example: Basic Rs.100 - Availability Penalty Rs.10 = Rs.90; MTTR cap = 25% of Rs.90.
    total_basic_sla = round(float(routes["SLA_Charges_Rs"].sum()), 2)
    mttr_cap_base_after_availability = round(total_basic_sla - availability_penalty_net, 2)
...
5) SLA Penalty Deduction (as per tender):
   ...
   MTTR Cap Base after Availability Penalty                        = Rs. {fmt_money(mttr_cap_base_after_availability)}
