...
def detect_fault_duration_column(df):
    ...
    raise ValueError(
        "Fault Duration column not found (need header containing 'Fault Duration' "
        "or at least 14 columns for Column N)."
    )
...
def robust_yes(val) -> bool:
    ...
    Accepts: YES / Y / TRUE / 1 / YES - unavoidable / Yes / EXEMPTED etc.
    Rejects: blank / NO / N / 0 / FALSE
    """
    if pd.isna(val):
        return False
...
def find_exemption_column(columns):
    ...
    - Any header containing 'exempt' or 'exemption' (covers your long header)
    - If still not found, look for headers mentioning mttr/availability + yes/no
    """
    cols = list(columns)
...
def process_sla(
    ...
    sla_month_text = pick_first_nonblank(routes["Month"].tolist())
    vendor_name = pick_first_nonblank(routes["Vendor_Name"].tolist())

    if sla_month_text == "":
        now = datetime.now()
        sla_month_text = f"{calendar.month_name[now.month]}-{now.year}"

    # month parse
    year, month = None, None
    m = re.search(r"(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)", str(sla_month_text).lower())
    y = re.search(r"(20\d{2})", str(sla_month_text))
    mon_map = {"jan":1,"feb":2,"mar":3,"apr":4,"may":5,"jun":6,"jul":7,"aug":8,"sep":9,"oct":10,"nov":11,"dec":12}
    if m:
        month = mon_map[m.group(1)]
    if y:
        year = int(y.group(1))
    ...
    manual_penalties_accounts = round(
        splice_loss_amt + supervisor_abs_amt + frt_abs_amt + petroller_abs_amt + relaying_not_done_amt + other_recovery, 2
    )
...
"""
    ...
    summary = pd.DataFrame([
        ...
        ["OA", oa_name],
        ["Vendor", vendor_name],
        ["SLA Month (Format-A)", sla_month_text],
