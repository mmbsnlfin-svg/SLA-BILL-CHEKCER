...
def create_pdf_note(output_path, title, text_body):
    """
    Create a clean, readable A4 portrait PDF note.
    Key totals are highlighted in red/bold and retention lines in blue/bold.
    """
    ...
    red_keywords = (
        ...
        "Total Exemption",
        "Net Penalty after exemption & MTTR cap",
        "Total Manual Penalties/Recoveries",
    )

    blue_keywords = (
        "Retention:",
        "Retention for 1% Re-laying work not done @200000/KM",
    )
    ...
    for line in lines:
        ...
        if any(k.lower() in line.lower() for k in blue_keywords):
            style = blue_style
        elif any(k.lower() in line.lower() for k in red_keywords):
            style = red_style
        else:
            style = normal_style

        story.append(Preformatted(line, style, maxLineLength=105))
...
def process_sla(
    ...
    # ---------- MTTR 25% cap ----------
    # Tender logic:
    # 1) 25% MTTR cap is calculated on:
    #       (Monthly service value - Availability penalty BEFORE exemption)
    # 2) Gross MTTR penalty cannot exceed this 25% cap.
    # 3) MTTR exemption marked by user unit is also restricted to this 25% cap.
    # 4) Net MTTR penalty = capped MTTR penalty - allowed MTTR exemption.
    ...
    # MTTR penalty before exemption, restricted to 25% cap
    mttr_penalty_before_exemption = round(mttr_gross, 2)
    mttr_penalty_after_cap_before_exemption = round(
        min(mttr_penalty_before_exemption, mttr_cap_25pct), 2
    )
    mttr_cap_applied = "YES" if mttr_penalty_before_exemption > mttr_cap_25pct else "NO"

    # MTTR exemption marked by user unit, also restricted to 25% cap
    mttr_exempt_marked = round(mttr_exempt, 2)
    mttr_exempt_allowed = round(
        min(mttr_exempt_marked, mttr_cap_25pct, mttr_penalty_after_cap_before_exemption), 2
    )
    mttr_exemption_limit_applied = "YES" if mttr_exempt_marked > mttr_exempt_allowed else "NO"

    # Net MTTR penalty after both cap and allowed exemption
    mttr_net_after_cap = round(
        max(mttr_penalty_after_cap_before_exemption - mttr_exempt_allowed, 0.0), 2
    )

    # Higher-of rule
    field_unit_penalty = float(field_unit_penalty or 0.0)
    system_sla_penalty_gross = round(mttr_gross + availability_penalty_gross, 2)
    system_sla_exemption_total = round(mttr_exempt_allowed + availability_penalty_exempt, 2)
...
5) SLA Penalty Deduction (as per tender):
   ...
   MTTR Penalty Before Exemption                                   = Rs. {fmt_money(mttr_penalty_before_exemption)}
   MTTR 25% Cap Amount                                             = Rs. {fmt_money(mttr_cap_25pct)}
   MTTR Exemption                                                  = Rs. {fmt_money(mttr_exempt_allowed)}
   Net MTTR Penalty                                                = Rs. {fmt_money(mttr_net_after_cap)}
   ...
   Net Availability Penalty                                        = Rs. {fmt_money(availability_penalty_net)}

   -------------------------------------------------------------------------------
   Total Penalty (MTTR + Availability)                             = Rs. {fmt_money(system_sla_penalty_gross)}
   Total Exemption                                                 = Rs. {fmt_money(system_sla_exemption_total)}{"   (25% Rule Applicable)" if mttr_exemption_limit_applied == "YES" else ""}
   Net Penalty after exemption & MTTR cap                          = Rs. {fmt_money(system_sla_penalty_net)}
   -------------------------------------------------------------------------------

   MTTR Cap Base after Availability Penalty Before Exemption       = Rs. {fmt_money(mttr_cap_base_after_availability)}
   MTTR Cap Applied                                                = {mttr_cap_applied}
   MTTR Exemption Marked by User Unit                              = Rs. {fmt_money(mttr_exempt_marked)}
   MTTR Exemption Limit Applied                                    = {mttr_exemption_limit_applied}
...
2. MTTR FAULTS Penalty Details                                   : (Code generated from Format-C)
   ...
   MTTR Cap Base after Availability Penalty Rs.                    : Rs. {fmt_money(mttr_cap_base_after_availability)}
   Max 25% MTTR Penalty Rs. ((RKM*Rate - Availability Penalty)*25%): Rs. {fmt_money(mttr_cap_25pct)}

   Slab wise faults and penalty:
{chr(10).join(mttr_lines)}

   MTTR Exemption Allowed after 25% limit                         : Rs. {fmt_money(mttr_exempt_allowed)}\n   Total MTTR Net penalty after Exemption (and 25% cap)           : Rs. {fmt_money(mttr_net_after_cap)}
...
"""
    ...
    summary = pd.DataFrame([
        ...
        ["MTTR cap base after availability penalty", mttr_cap_base_after_availability],
        ["MTTR cap 25%", mttr_cap_25pct],
        ["MTTR cap applied", mttr_cap_applied],
        ["MTTR gross", mttr_gross],
        ["MTTR exemption marked by user unit", mttr_exempt_marked],
        ["MTTR exemption allowed after 25% limit", mttr_exempt_allowed],
        ["MTTR exemption limit applied", mttr_exemption_limit_applied],
        ["MTTR penalty before exemption", mttr_penalty_before_exemption],
        ["MTTR penalty after 25% cap before exemption", mttr_penalty_after_cap_before_exemption],
        ["MTTR net after cap and allowed exemption", mttr_net_after_cap],
